...
namespace Controls
{
    public class Ckeditor : HtmlTextArea
    {
        ...
        protected override void OnPreRender(EventArgs e)
        {
            ...
            Page.ClientScript.RegisterClientScriptInclude("ckeditorScript", VirtualPathUtility.AppendTrailingSlash(ResolveUrl(BaseHref)) + "ckeditor.js");
            Page.ClientScript.RegisterStartupScript(Page.GetType(), "ckeditorInit", Regex.Replace(initCkeditor, @"\s{2,}", " "), true);
            Page.ClientScript.RegisterStartupScript(Page.GetType(), "ckeditorLoad" + UniqueID, "initCKeditor('" + ClientID + "', { " + String.Concat(", ", settings) + " });", true);
            Page.ClientScript.RegisterOnSubmitStatement(this.GetType(), "ckeditorSave" + UniqueID, "CKEDITOR.instances['" + ClientID + "'].updateElement();");
